  ...
  <b-container fluid class="main-container my-3">
    ...
    <b-row>
      ...
      <b-col class="text-right">
        <b-button
          v-on:click="modificaCategorieBulk()"
          class="mr-2 btn-outline-secondary btn-sm"
          >Assegna Categoria</b-button
        >
        <b-button v-on:click="stampaBulk()" class="btn-outline-secondary btn-sm"
          >Stampa</b-button
        >
      </b-col>
    </b-row>
    ...
    <b-row class="secound-row my-2">
      <b-col cols="8" class="text-left">
        <b-button
          v-on:click="selezionaTutto(1)"
          variant="success"
          class="btn-outline-success btn-sm"
          >Seleziona Tutto</b-button
        ><b-button
          variant="danger"
          v-on:click="selezionaTutto(0)"
          class="ml-2 btn-outline-danger btn-sm"
          >Deseleziona Tutto</b-button
        >
      </b-col>
      <b-col cols="2" class="pr-0 text-right"> Filtra Categoria </b-col>
      <b-col class="text-right">
        <model-select
          id="filtraCat"
          class="form-control"
    ...
    <b-table
      ...
      :fields="[
        ...
        { key: 'ct', label: 'Carati', sortable: true, sortDirection: 'desc' },
        { key: 'foto', label: 'Foto' },
      ]"
    >
      <template #cell(x)="row">
      ...
      <template #cell(qr)="row">
        <a
          :href="
            ...
            $store.state.settings.URL_WEBAPP +
            '%23%2Fqr%2F' +
            row.item.codice +
            '&code=Code128&translate-esc=on&quiet=1'
          "
          target="_blank"
          >1D <b-icon icon="upc" font-scale="2"></b-icon></a
        ><br />
        <a
          :href="
            'https://barcode.tec-it.com/barcode.ashx?data=' +
            ...
            '%23%2Fqr%2F' +
            row.item.codice +
            '&code=MobileQRCode&translate-esc=on&quiet=1'
          "
          target="_blank"
          >QR <b-icon icon="upc" font-scale="2"></b-icon
        ></a>
      </template>
      <template #cell(categorie)="row">
        <a v-b-modal.modal-aggiungiCategoria @click="sendInfo(row.item)"
          >Aggiungi</a
        >
        <p v-bind:key="categoria.index" v-for="categoria in row.item.categorie">
          ...
          <img
            ...
            width="20"
            alt="Vue logo"
            @click="deleteCatProd(categoria.idCat, row.item.id)"
            src="../assets/del.png"
          />
        </p>
      </template>
      <template #cell(foto)="row">
        ...
        <p v-bind:key="img.index" v-for="img in row.item.imgs">
          ...
          <img
            ...
            width="20"
            alt="Vue logo"
            @click="deleteImage(img, row.item.id)"
            src="../assets/del.png"
          />
        </p>
      </template>
    ...
    <div>
      <b-modal
        id="modal-1"
        hide-footer
        :title="'CARICA IMMAGINE: ' + selectedProduct.titolo"
        size="xl"
      >
        <b-form>
    ...
    >
      <b-form inline>
        <label class="mr-sm-12" for="inline-form-custom-select-pref">
          ...
          <b-form-select
            id="aggiungiCatAProd"
            name="aggiungiCatAProd"
            v-model="selectedCategoria"
            :options="optionsCategorie"
            ref="selectedCategoria"
          ></b-form-select>
        </label>
        <br /><br />
        <div v-if="showUploadingSpinner">
    ...
    <b-modal
      id="modal-aggiungiCategoriaBulk"
      hide-footer
      :title="'NUOVA CATEGORIA - ASSOCIAZIONE MULTIPLA'"
      size="xl"
    >
      ...
      <b-form inline>
        <label class="mr-sm-12" for="inline-form-custom-select-pref">
          Seleziona categoria
          <b-form-select
            id="aggiungiCatAProdBulk"
            name="aggiungiCatAProdBulk"
            v-model="selectedCategoriaBulk"
            :options="optionsCategorie"
            ref="selectedCategoriaBulk"
          ></b-form-select>
        </label>
        <br /><br />
        <div v-if="showUploadingSpinner">
    ...
    <form
      :action="$store.state.settings.URL_SERVER + 'API/zebra/'"
      method="post"
      target="_blank"
      ref="formCodaStampa"
      id="formCodaStampa"
...
export default {
  ...
  methods: {
    ...
    modificaCategorieBulk() {
      this.nCheck = 0;
      for (let key in this.checkProdotto) {
        if (this.checkProdotto[key] == true) this.nCheck = this.nCheck + 1;
      }
    ...
    submitNewCatBulk() {
      ...
      let idProdotti = [];
      for (let key in this.checkProdotto) {
        if (this.checkProdotto[key] == true) idProdotti.push(key);
      }
      this.showUploadingSpinner = true;
      ...
      axios
        .get(
          this.$store.state.settings.URL_SERVER +
            "API/v1.php?prodotti&thumb=small"
        )
        .then((response) => {
    ...
    updateCategoriesList() {
      axios
        .get(
          this.$store.state.settings.URL_SERVER +
            "API/v1.php?categorie&thumb=small"
        )
        .then((response) => {
          ...
          response.data.categorie.map(function (value) {
            if (value.livello == 2) {
              lista.push({
                value: value.id,
                text: value.genitore_nome + "->" + value.nome,
              });
            }
          });
    ...
    submitFile() {
      ...
      axios
        .post(
          this.$store.state.settings.URL_SERVER + "API/upload.php",
          formData,
          {
            headers: {
              "Content-Type": "multipart/form-data",
