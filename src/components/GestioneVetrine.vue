<template>
  <b-container fluid>
    ...
    <b-table striped hover :fields="fields" :items="vetrine">
      <template #cell(immagine)="row">
        <b-img
          :src="row.item.img + '?' + $session.get('secret')"
          width="100"
        ></b-img>
      </template>
      ...
      <template #cell(azione)="row">
        <b-button
          ...
          class="mr-1"
          variant="primary"
          @click="modificaVetrina(row.item)"
          >Modifica Prodotti</b-button
        >
        <b-button
          size="sm"
          class="ml-1"
          variant="outline-danger"
          @click="eliminaProdotti(row.item.id)"
          >Rimuovi tutti i prodotti</b-button
        >
        <b-button
          size="sm"
          class="ml-2"
          variant="warning"
          @click="modificaFotoVetrina(row.item)"
          >Modifica Foto</b-button
        >
        <b-button
          size="sm"
          class="ml-2"
          variant="danger"
          @click="eliminaVetrina(row.item.id)"
          >Elimina</b-button
        >
      </template>
    </b-table>
    ...
    <b-modal
      id="modifica-vetrina"
      hide-footer
      :title="inModifica.nome"
      size="xl"
    >
      <div class="insideWrapper" id="canvas-wrapper">
        <b-img
          :src="inModifica.img + '?' + $session.get('secret')"
          fluid
          @load="creaCanvas"
          id="immagineVetrina"
        ></b-img>
        <canvas
          ...
          v-on:click="clickVetrina"
        ></canvas>
      </div>
    </b-modal>

    <b-modal
      id="modal-prodotto-vetrina"
      hide-footer
      :title="titoloModalPunto"
      size="xl"
    >
        ...
        <b-row>
          <b-col>
            <model-select
              ...
              v-model="puntoSelezionato.puntoProdottoVetrina"
              placeholder="Seleziona il prodotto"
            ></model-select>
          </b-col>

          <b-button
            v-if="puntoSelezionato.id == null"
            type="button"
            @click="salvaPunto()"
            variant="success"
            >SALVA</b-button
          ...
          <b-button
            v-if="puntoSelezionato.id != null"
            type="button"
            @click="eliminaPunto(puntoSelezionato.id)"
            variant="danger"
            class="ml-1"
            >ELIMINA</b-button
          >
          ...
          <b-row>
            <b-col>
              <label class="mr-sm-12" for="inline-form-custom-select-pref">
                ...
                <b-form-input
                  id="nomeNuovaVetrina"
                  name="nomeNuovaVetrina"
                  v-model="nomeNuovaVetrina"
                  ref="nomeNuovaVetrina"
                ></b-form-input>
              </label>
            </b-col>
            <b-col>
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        { key: "immagine", sortable: false },
        { key: "nome", sortable: true },
        { key: "nomeCat", sortable: true },
        { key: "nProdottiAssociati", sortable: true },
        { key: "azione", sortable: false },
      ],
      vetrine: [],
      file: "",
      files: [],
  ...
  methods: {
    creaCanvas() {
      ...
      this.VetrinaDim.maxX =
        document.getElementById("immagineVetrina").clientWidth;
      this.VetrinaDim.maxY =
        document.getElementById("immagineVetrina").clientHeight;
      this.ctx = c.getContext("2d");
      this.ctx.canvas.width = this.VetrinaDim.maxX;
      this.ctx.canvas.height = this.VetrinaDim.maxY;
      console.log(
        "ctx:" + this.ctx.canvas.width + "," + this.ctx.canvas.height
      );
    ...
    clickVetrina() {
      ...
      for (let i = 0; i < this.vetrinaProdotti.length; i++) {
        ...
        if (
          dx * dx + dy * dy <=
          (this.raggioPunto / this.VetrinaDim.maxX) *
            (this.raggioPunto / this.VetrinaDim.maxX)
        ) {
          this.puntoSelezionato.puntoProdottoVetrina =
            this.vetrinaProdotti[i].idProdotto;
      ...
      axios
        .get(
          this.$store.state.settings.URL_SERVER +
            "API/v1.php?eliminaPuntoVetrina=" +
            idPunto +
            "&secret=" +
    ...
    disegnaAreaProdotto(p) {
      ...
      this.ctx.beginPath();
      this.ctx.arc(
        p.x * this.VetrinaDim.maxX,
        p.y * this.VetrinaDim.maxY,
        this.raggioPunto,
        0,
        2 * Math.PI,
        false
    ...
    getListaCategorie() {
      ...
      axios
        .get(
          this.$store.state.settings.URL_SERVER +
            "API/v1.php?categorieLivello=2&thumb=medium"
        )
        .then((response) => {
          console.log(response);
          let lista = [];
        ...
        callback: (confirm) => {
          if (confirm) {
            ...
            axios
              .get(
                this.$store.state.settings.URL_SERVER +
                  "API/v1.php?eliminaProdottiVetrina=" +
                  idVetrina +
                  "&secret=" +
                  this.$session.get("secret")
              )
