    ...
    <b-col>
      <b-row>
        <b-col>
          <div class="mx-auto mb-3 mt-1">
            <b-button v-b-modal.modal-nuovoLAB style="float:right; margin-left:2em;margin-bottom:1em" variant="success"
              @click="clearformNuovaLavorazione()" class="btn-outline-success btn-sm"><b-icon
                icon="plus"></b-icon>NUOVO</b-button>

          </div>
        </b-col>
        <b-col>
          <a v-if="this.$session.get('AUTORIZZAZIONI').numeri_telefono >= 1" style="float:right"
            v-bind:href="$store.state.settings.URL_SERVER + 'API/esportaXLSX.php?esportaLaboratorio&secret=' + $session.get('secret')"
            target="_blank">
      ...
      <table-component :data="laboratorio" responsive @rowClick="dettaglioRiparazione" filter-input-class="classeFiltro">
        ...
        <table-column header-class="w-5" label="Azioni    ">
          <template slot-scope="row">
            <b-row>
              <img class="azione" width="20" alt="EditN" @click.stop="copiaPerStampa(row.id)"
              ...
              <img class="azione" width="20" alt="EditN" @click.stop="modificaRiparazione(row)"
                src="../assets/detail.png" />
              <a href="mailto:" @click.stop=""><img class="azione" width="20" alt="email" src="../assets/mail.png" /></a>


              <img class="azione" width="20" alt="x" @click.stop="eliminaRiparazione(row.id)" src="../assets/del.png"
     ...
     <table-column show="pietre" label="Pietre"></table-column> -->
        ...
        <table-column show="flagd" label="Prog">
          <template slot-scope="row">
            <a @click.stop="visualizzaProgetto(row)" v-if="row.imgs.length > 0"><b-icon icon="card-image" font-scale="1"
                style="margin-left:0.5em"></b-icon></a>
          </template>
        </table-column>
            ...
            <b-row>
              ...
              <b-col>
                <b-form-group label="Ricevuto:" label-for="input-data">
                  <b-form-datepicker id="input-data" today-button reset-button v-model="formNuovoLAB.data"
                    class="mb-2"></b-form-datepicker>
                </b-form-group>
              </b-col>
              <b-col>
                <b-form-group label="Quantità:" label-for="input-qnt">
                  <b-form-input @focus.native="$event.target.select()" id="input-qnt" v-model="formNuovoLAB.qnt"
                    type="number" step="1"></b-form-input>
                </b-form-group>
              </b-col>
              <b-col>
                <b-form-group label="Carati:" label-for="input-carati">
                  <b-input-group append="ct">
                    <b-form-input @focus.native="$event.target.select()" id="input-qnt" v-model="formNuovoLAB.carati"
            ...
            <b-row>
              <b-col>
                <b-form-group label="Tipo:" label-for="input-tipo">
                  <b-form-input @focus.native="$event.target.select()" id="input-tipo" v-model="formNuovoLAB.tipo"
                    type="text"></b-form-input>
                </b-form-group>
              </b-col>
              <b-col>
                <b-form-group label="Caratteristiche:" label-for="input-caratteristiche">
                  <b-form-input @focus.native="$event.target.select()" id="input-caratteristiche"
                    v-model="formNuovoLAB.caratteristiche" type="text"></b-form-input>
                </b-form-group>
              </b-col>
              <b-col>
                <b-form-group label="Grammi:" label-for="input-grammi">
                  <b-input-group append="g">
                    <b-form-input @focus.native="$event.target.select()" id="input-grammi" v-model="formNuovoLAB.grammi"
                      type="text"></b-form-input>
                  </b-input-group>
                </b-form-group>
              </b-col>
            ...
            <b-row>
              <b-col>
                <b-form-group label="Costo Incassatura:" label-for="input-cIncassatura">
                  <b-form-input id="input-cIncassatura" v-model="formNuovoLAB.cIncassatura" type="number"
                    step=".01"></b-form-input>
                </b-form-group>
              </b-col>
              <b-col>
                <b-form-group label="Costo Fusione:" label-for="input-cFusione">
                  <b-form-input id="input-cFusione" v-model="formNuovoLAB.cFusione" type="number"
                    step=".01"></b-form-input>
                </b-form-group>
              </b-col>
              <b-col>
              ...
              <b-col>
                <b-form-group label="Costo Disegno:" label-for="input-cDisegno">
                  <b-form-input id="input-cDisegno" v-model="formNuovoLAB.cDisegno" type="number"
                    step=".01"></b-form-input>
                </b-form-group>
              </b-col>
              <b-col>
            ...
            <b-container>
              <b-row>
                ...
                <b-col v-bind:key="file.index" v-for="(file) in formNuovoLAB.fileCaricati">
                  <b-img v-if="isImage(file.nome)" :src="file.src + '?' + $session.get('secret')" :alt="file.nome" fluid
                    @click="zoom(file.src)"></b-img>
          ...
          {{ file.nome }}   <img class="azione" width="20" alt="X"  @click="removeFileCaricato(file.id)" src="../assets/del.png">
              ...
              <b-col>
                <div class="uploaderDiv" v-cloak @drop.prevent="addFile" @dragover.prevent>
                  ...
                  <p>Carica Immagini</p><button type="button" @click="$refs.file.click()">Apri finestra o
                    Trascina</button>

                  <ul>
                    <li v-bind:key="file.index" v-for="file in files">
                      {{ file.name }} ({{ file.size | bTokb }} kb) <img class="azione" width="20" alt="X"
                        @click="removeFile(file)" src="../assets/del.png">
                    </li>
                  </ul>
...
<style>.btn-outline-danger:hover {
