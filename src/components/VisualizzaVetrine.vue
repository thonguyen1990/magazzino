<template>
  <b-container fluid>
    ...
    <template #cell(azione)="row">
      <b-button size="sm" class="mr-1" @click="modificaVetrina(row.item)">Modifica</b-button>
  ...
  <b-modal id="modifica-vetrina" hide-footer :title="inModifica.nome" size="xl">
    <div class="insideWrapper" id="canvas-wrapper">
      <b-img :src="inModifica.img+'?'+$session.get('secret')+'&'+timestamp" fluid @load="creaCanvas" id="immagineVetrina"></b-img>
...
export default {
  ...
  data() {
    return {
      ...
      fields:[  { key: 'immagine', sortable: false },{ key: 'nome', sortable: true },{ key: 'nProdottiAssociati', sortable: true },{ key: 'azione', sortable: false }],
      vetrine:[],
      file: '',
      files:[],
      fileImage:'',
