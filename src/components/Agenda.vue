        ...
        <b-row>
          <b-col>
            <b-form-group label="Titolo:" label-for="input-titolo">
              <b-form-input id="input-titolo" v-model="nuovoEvento.title" type="text"
                :disabled="nuovoEvento.disabilitato"></b-form-input>
            </b-form-group>
            <b-form-group label="Descrizione:" label-for="input-descrizione">
              <b-form-textarea id="input-descrizione" rows="3" v-model="nuovoEvento.description" type="text"
                :disabled="nuovoEvento.disabilitato"></b-form-textarea>
            </b-form-group>
          </b-col>
        </b-row>
        <b-row>
          <b-col>
            <b-form-group label="Da:" label-for="input-inizio">
              <b-form-datepicker id="input-inizio" :max="nuovoEvento.dayEnd" :disabled="nuovoEvento.disabilitato"
                today-button reset-button v-model="nuovoEvento.dayStart" class="mb-2"
                :state="validaForm.dayStart"></b-form-datepicker>
            </b-form-group>
          </b-col>
          <b-col>
            <b-form-group label="A:" label-for="input-fine">
              <b-form-datepicker id="input-fine" :min="nuovoEvento.dayStart" :disabled="nuovoEvento.disabilitato"
                today-button reset-button v-model="nuovoEvento.dayEnd" class="mb-2"
                :state="validaForm.dayEnd"></b-form-datepicker>
            </b-form-group>
          </b-col>
          <b-col>
            <b-form-group label="Notifica:" label-for="notifica">
              <model-select id="notifica" class="form-control" :options="optionsNotifica" v-model="nuovoEvento.notifica">
              </model-select>
            </b-form-group>
          </b-col>
        </b-row>
        <span v-if="!nuovoEvento.disabilitato">
          <b-button v-if="nuovoEvento.id > 0" style="float:right" type="button" variant="danger"
            @click="eliminaEvento(nuovoEvento.id)">ELIMINA</b-button>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      nuovoEvento: { id: 0, dayStart: null, dayEnd: null, title: '', description: '', allDay: true, tipo: 'AGENDA', idRelativo: 0, disabilitato: false, notifica: 0 },
      validaForm: { dayStart: null, dayEnd: null },
      optionsNotifica: [{ value: 0, text: "NO" }, { value: 1, text: "Adesso" }, { value: 2, text: "Stesso giorno" }, { value: 3, text: "1 Giorno prima" }, { value: 4, text: "2 Giorni prima" }, { value: 5, text: "3 Giorni prima" },
      { value: 6, text: "4 Giorni prima" }, { value: 7, text: "5 Giorni prima" }, { value: 8, text: "6 Giorni prima" }, { value: 9, text: "7 Giorni prima" }],
      isLoading: false,
    }
  },
  mounted() {
  ...
  methods: {
    getEventi() {
      this.isLoading = true;
      axios
        .get(this.$store.state.settings.URL_SERVER + "API/v1.php?listaEventi&secret=" + this.$session.get('secret'))
        .then(response => {
          ...
          response.data.eventi.map(function (value) {
            ...
            lista.push({ id: value.id, title: value.title, description: value.description, start: value.dayStart, end: dateEnd, allDay: true, tipo: value.tipo, color: "#03373a", notifica: value.notifica });
          });
    ...
    handleClickEvent(arg) {
      ...
      this.nuovoEvento.disabilitato = (arg.event.extendedProps.tipo == 'AGENDA') ? false : true;
      ...
      this.$confirm(
        {
          ...
          */
          callback: confirm => {
            if (confirm) {
              axios
                .get(this.$store.state.settings.URL_SERVER + "API/v1.php?eliminaEvento&id=" + id + "&secret=" + this.$session.get('secret'))
                .then(response => {
    ...
    onSubmitNuovoEvento() {
      ...
      axios.post(this.$store.state.settings.URL_SERVER + "API/v1.php", this.toFormData(this.nuovoEvento), {
        headers: {
