<template>
  <b-container fluid>
    ...
    <b-button type="button" @click="esportaDBintero()" variant="primary"
      >BACKUP DATABASE COMPLETO</b-button
    >
    <hr />
    <b-row>
      <b-col>
        ...
        <b-form @submit="onSubmitModificaDB">
          ...
          <b-form-group label="A:" label-for="input-formA">
            <b-form-datepicker
              ...
              today-button
              reset-button
              v-model="formDB.dataFine"
              class="mb-2"
            ></b-form-datepicker>
          </b-form-group>
          <b-button type="submit" variant="primary"
            >SCARICA DATI DAL DATABASE</b-button
          >
          <b-button type="button" variant="danger" @click="eliminaDati()"
            >ELIMINA DATI DAL DATABASE</b-button
          >
          <b-button type="button" variant="" @click="resetForm()"
            >PULISCI FORM</b-button
          >
        </b-form>
      </b-col>
    </b-row>
    ...
    <div style="float: right">
      <b-icon
        icon="shield-fill"
        font-scale="2"
        style="margin-right: 0.5em; margin-top: 2em"
        @click="reset()"
      ></b-icon>
    </div>
  </b-container>
  ...
  data() {
    return {
      ...
      optionsTipo: [
        {
          value: "VV",
          text: "Visite e Vendite [in base alla data di vendita/visita]",
        },
        {
          value: "LAVORAZIONI",
          text: "Lavorazioni [SOLO consegnati e in base alla data di ultima modifica]",
        },
      ],
      file: "",
    };
  ...
  methods: {
    ...
    eliminaDati() {
      ...
      if (tabella != "") {
        this.$confirm({
          message:
            "ATTENZIONE!! Eliminare tutti i dati ( " +
            tabella +
            " ) [DA: " +
            this.formDB.dataInizio +
            " A: " +
            this.formDB.dataFine +
            "]?",
          button: {
            no: "No",
            yes: "Si",
